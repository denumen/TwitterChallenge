...
# Define search query to recieve all tweets to TaylorLorenz account
    taylorlorenz_query = 'to:taylorlorenz filter:replies'
    # Retrieve tweets
    tweets = []
    for i, tweet in enumerate(sntwitter.TwitterSearchScraper(taylorlorenz_query).get_items()):
    ...
    # Define search query to recieve all tweets from TaylorLorenz account
    taylorlorenz_query = 'from:taylorlorenz'
    # Retrieve tweets
    tweets = []
    for i, tweet in enumerate(sntwitter.TwitterSearchScraper(taylorlorenz_query).get_items()):
        if tweet.date < time_since: # Stop when tweets are older than time window
...
while True:
    ...
    # df_tweets_base has tweets in reply to TaylorLorenz
    # df_tweets_owner_base has tweets from TaylorLorenz
    if First_run == True :
        ...
        df_tweets_base.to_csv("~/taylorlorenz_base.csv", index=False)
        df_tweets_owner_base.to_csv("~/taylorlorenz_owner_base.csv", index=False)
        print(First_run)
        First_run = False
        time.sleep(400)
    else:
        ...
        # taylorlorenz_all has the old data and new data in reply to TaylorLorenz
        # taylorlorenz_owner_all has the old data and new data from TaylorLorenz
        taylorlorenz_all = pd.concat([df_tweets, df_tweets_base])
        taylorlorenz_owner_all = pd.concat([df_tweets_owner, df_tweets_owner_base])
        taylorlorenz_all = taylorlorenz_all.reset_index(drop=True)
        taylorlorenz_owner_all = taylorlorenz_owner_all.reset_index(drop=True)
        ...
        taylorlorenz_all = taylorlorenz_all.drop_duplicates(subset=['id'])
        taylorlorenz_owner_all = taylorlorenz_owner_all.drop_duplicates(subset=['id'])
        # we replace the old data with new one in each run, so we can concat new data in next runs and don't miss any data
        df_tweets_base = taylorlorenz_all.copy()
        df_tweets_owner_base = taylorlorenz_owner_all.copy()
        # Saving data to process them in process_tweets.py
        taylorlorenz_all.to_csv("~/taylor_all.csv", index=False)
        taylorlorenz_owner_all.to_csv("~/taylor_owner_all.csv", index=False)
        # Print debug output
        #print('hi')
